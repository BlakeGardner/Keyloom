...
- **Mac shortcuts, offered when an Apple keyboard is connected.** When a
  genuine Apple keyboard is recognised (clones borrowing Apple's vendor id
  are already told apart), at launch or when it is plugged in, the Keyboard
  view shows a card above the deck that names the keyboard and offers Mac
  shortcuts on it: one action-named button and a "Not now". Accepting adds
  a set of shortcut groups to the active profile, limited to that keyboard,
  so the profile's other remaps and every other keyboard behave as before,
  and the toast's Undo takes the set back out. "Not now" is remembered for
  that keyboard, and the set can still be added later from the Shortcuts
  view. Shortcut groups need a keyboard scope first (they take an
  application scope only; mappings already generate xremap's `device`
  filter), and the card has to say when this desktop's xremap cannot tell
  which window is in front, since shortcuts that differ per application
  depend on it.
